...
@Configuration()
class GetDefender(GeneratingCommand):
    ...
    def generate(self):
        ...
        try:
            ...
            if instance_role in ('splunk_cloud'):
                ...
                # run call
                try:
                    response = circ_relay_get_status(self.account, relay_token, self.computername, relay_url)
                    yield_record = {
                    '_time': time.time(),
                    '_raw': response,
                    }
                ...
                yield_record = {
                    ...
                    '_raw': {
                        ...
                        'computername': self.computername,
                        'account': self.account,
                        'response': 'sending get_defender_status request to circ=\"{}\"'.format(circ_url),
                    },
                }
                logging.info(json.dumps(yield_record, indent=2))
                yield yield_record
                ...
                # run call
                try:
                    response = circ_get_status(circ_token, self.computername, circ_url)
                    yield_record = {
                    '_time': time.time(),
                    '_raw': response,
                    }
            ...
            msg = "get_defender_status, an exception was encountered, exception=\"{}\"".format(str(e))
            yield {
                '_raw': msg,
                '_time': time.time()
                }
...
dispatch(GetDefender, sys.argv, sys.stdin, sys.stdout, __name__)
